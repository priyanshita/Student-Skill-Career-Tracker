...
### Q1: How did you implement Inheritance and Abstraction in your project?
**Answer**: I created an abstract base class named `Skill` that defines common attributes (`id`, `name`, `proficiency`, `category`) and abstract methods (`getProficiencyWeight()`, `getDetails()`, `getSkillType()`). Concrete classes `TechnicalSkill` and `SoftSkill` inherit from `Skill` using the `extends` keyword, invoke the parent constructor with `super()`, and provide domain-specific implementations for the abstract methods.

### Q2: Where is Polymorphism demonstrated in your application?
**Answer**: Polymorphism is demonstrated in the `SkillService` and `SkillGapAnalyzer` classes, where collections of type `List<Skill>` contain both `TechnicalSkill` and `SoftSkill` instances. When calling `skill.getProficiencyWeight()` or `skill.toDataString()`, Java's dynamic method dispatch automatically calls the appropriate overridden method at runtime without requiring explicit type checking.

### Q3: How is data persisted in your application without using an external SQL database?
**Answer**: I implemented custom text-file storage using `FileManager` with Java's `BufferedReader`, `BufferedWriter`, `FileReader`, and `FileWriter`. Objects implement the `Storable` interface, converting state to pipe-delimited strings (`|`). `FileManager` parses these text lines line-by-line and handles missing or corrupted file records gracefully.

### Q4: Explain the Career Readiness calculation algorithm. Is it hardcoded?
**Answer**: No, it is not hardcoded. The calculation is performed dynamically by `SkillGapAnalyzer` using a weighted formula:
$$\text{Readiness} = (0.50 \times \text{SkillScore}) + (0.25 \times \text{ProjectScore}) + (0.15 \times \text{CertScore}) + (0.10 \times \text{BreadthScore})$$
`SkillScore` compares the student's acquired skills against the required skills for their chosen `CareerRole`, scaling by proficiency weights (Advanced = 1.0, Intermediate = 0.7, Beginner = 0.4).

### Q5: How do you prevent duplicate skills from being added to a student's profile?
**Answer**: The `Skill` class overrides `equals()` and `hashCode()` to perform case-insensitive name comparisons. When adding a new skill, `SkillService.addSkill()` iterates through existing skills and checks `existing.equals(newSkill)`. If a match is found, it throws a custom `DuplicateSkillException`.

### Q6: What custom exceptions did you create and why?
**Answer**: I created a base exception class `TrackerException` extending `Exception`, and specific sub-exceptions: `InvalidStudentDataException` for validation errors, `DuplicateSkillException` for duplicate entries, `SkillNotFoundException` for lookup failures, and `DataPersistenceException` for file I/O issues. This ensures clean error separation and informative UI feedback.

### Q7: What searching and sorting algorithms are included?
**Answer**: In `AlgorithmUtil`, I implemented **MergeSort** (a recursive divide-and-conquer sorting algorithm) to order skills by proficiency weight descending, as well as **Binary Search** (`O(log n)` time complexity) to quickly find skills by name in an alphabetically sorted list.

### Q8: How did you design the user interface?
**Answer**: The desktop GUI is built with **JavaFX**. It features a `BorderPane` root layout with a fixed sidebar navigation menu controlling dynamic scene switching between 8 specialized views (`DashboardView`, `ProfileView`, `SkillsView`, `ProjectsView`, `CertificationsView`, `CareerGoalsView`, `SkillGapView`, `ProgressReportView`). Visual styling is managed via `styles.css`.

### Q9: How are target career roles structured?
**Answer**: Target roles are encapsulated in the `CareerRole` model, which maintains lists of `requiredSkills` and `recommendedSkills`. The application comes with 6 pre-configured roles (*AI/ML Engineer*, *Software Developer*, *Data Analyst*, *Data Scientist*, *Web Developer*, *Cloud Engineer*) and allows students to define custom roles dynamically.

### Q10: How do unit tests verify your application logic?
**Answer**: Using **JUnit 5**, I created automated unit test classes (`SkillServiceTest`, `SkillGapAnalyzerTest`, `FileManagerTest`, `ValidationAndAlgorithmTest`). These tests programmatically verify skill addition, duplicate prevention, skill gap classification, readiness formula evaluation, file I/O roundtrips, and search/sort correctness without needing to manually launch the GUI.
